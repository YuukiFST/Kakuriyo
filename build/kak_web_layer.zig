...
//! (the vendored sibling build/kak_app.zig imports this instead of the
//! SDK-internal relative path). Re-vendor together with kak_app.zig.
...
/// Test-only shape doubles for the typed-Manifest contract tests below
/// (upstream imports these from types.zig; this vendored copy keeps the
/// tests but defines the minimal shapes they exercise — the scanning
/// functions are duck-typed, so the doubles and the real types agree
/// row-for-row on the contract matrix).
const types = struct {
    pub const Capability = enum { webview, native_views, gpu_surfaces };
    pub const ViewKind = enum { gpu_surface, toolbar, webview };
    pub const ShellView = struct { label: []const u8, kind: ViewKind };
    pub const ShellWindow = struct { label: []const u8, views: []const ShellView };
    pub const Shell = struct { windows: []const ShellWindow = &.{} };
    pub const Manifest = struct {
        identity: struct { id: []const u8, name: []const u8 },
